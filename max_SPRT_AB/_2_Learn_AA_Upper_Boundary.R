...
# Calculate boundary for p1 value 
...
DF = read.csv(gzfile(paste0("AB_",p1,"-",p1 ,"-",N[1] ,".gz")))

names (DF) = c("numSim","numIte","Arm1_suc","Arm2_suc","Arm1_cum_suc","Arm2_cum_suc")

yx  = mclapply(1:1000, AA_LLR_AB_AllSimulation_vector , DF_=DF ,N =N[1] , mc.cores = 10)
x   = do.call(rbind.data.frame, yx)

names (x) = 1:length(x)
write.csv(x, paste0("AB_",p1,"-",p1,"-","_LLR_AA_Boundry-.csv"))
...
batch = vector ()
q     = vector ()
df = read.csv(paste0("AB_",p1,"-",p1,"-","_LLR_AA_Boundry-.csv"))
